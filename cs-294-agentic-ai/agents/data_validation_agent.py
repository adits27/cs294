...
from .storage import resolve_path, is_r2_path
...
class DataValidationAgent(BaseAgent):
    ...
    def process_request(self, message: A2AMessage) -> A2AMessage:
        ...
        if data_source:
            ...
            if is_r2_path(data_source):
                ...
                if '*' in data_source:
                    ...
                    try:
                        # The resolve_path function will download from R2
                        # For patterns, we'll try the base path first
                        ...
                        if r2.is_configured():
                            import tempfile
                            temp_dir = tempfile.mkdtemp(prefix='r2_data_')
        ...
        logger.info(f"Starting data validation for dataset: {local_data_path}")

        tool_result = self._validate_with_tool(local_data_path, ab_test_context)
        ...
        if tool_result["success"]:
            ...
            analysis = self._fallback_to_llm(local_data_path, ab_test_context, tool_result["error"])
            method = "llm_fallback"
